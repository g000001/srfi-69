...
(define-function (hash obj . maybe-bound)
  (let ((bound (if (null? maybe-bound) *default-bound* (car maybe-bound))))
    (cond ((null? obj) 0)
          ((integer? obj) (modulo (rational obj) bound))
          ((string? obj) (string-hash obj bound))
          ((symbol? obj) (symbol-hash obj bound))
          ((real? obj) (let ((obj (rational obj)))
                         (modulo (+ (numerator obj)
                                    (denominator obj))
                               bound)))
          ((number? obj)
          ...
          ((pair? obj) (if (or (atom (cdr obj))
                               (list-length obj))
                           (modulo (+ (hash (car obj)) (* 3 (hash (cdr obj))))
                                   bound)
                           ;; FIXME circular-list
                           (modulo (+ (hash (car obj)) (* 3 (sxhash (cdr obj))))
                                   bound)) )
          ((not obj) 0)
